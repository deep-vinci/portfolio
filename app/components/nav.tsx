...
export function Navbar() {
    ...
    return (
        <div className="fixed bottom-4 sm:bottom-5 left-1/2 -translate-x-1/2 z-50">
            <nav
                className="relative overflow-hidden flex items-center gap-1 sm:gap-2 bg-[#161616]/95 backdrop-blur-xl rounded-full px-2 py-2 shadow-2xl shadow-black/60"
                style={{
                    backgroundImage:
            ...
            >
                {links.map((link) => {
                    ...
                    return (
                        <Link
                            key={link.href}
                            href={link.href}
                            className={`relative flex items-center gap-2 text-xs font-mono font-bold tracking-widest uppercase px-4 sm:px-5 py-2 rounded-full transition-colors duration-300 ${
                                active
                                    ? "text-white"
                ...
                {/* Copyright */}
                <div className="hidden md:flex items-center gap-2 text-[10px] font-mono tracking-widest text-[#898989] uppercase font-semibold border-l border-[#2e2e2e] pl-4 pr-3 ml-1">
